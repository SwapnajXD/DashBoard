...
export function LiveLogs() {
  ...
  return <section className="collection live-logs" aria-labelledby="live-logs-title">
    <div className="collection-heading"><h3 id="live-logs-title">Live logs <Badge label={label} tone={paused || error ? "warn" : connecting ? "muted" : "good"}/></h3><button className="log-toggle" onClick={() => setPaused(value => !value)}>{paused ? <Play size={14}/> : <Pause size={14}/>} {paused ? "Resume" : "Pause"}</button></div>
    <div className="log-controls"><label className="search-field"><Search size={15}/><span className="sr-only">Filter displayed log messages and sources</span><input type="search" placeholder="Filter messages or sources" value={filter} onChange={event => setFilter(event.target.value)}/></label><span>Newest first · latest 100 entries / 15 minutes</span></div>
    <p className="log-status" role="status">{error ? `${error}${snapshot ? " Displayed entries are from the last successful refresh." : ""}` : paused ? "Updates paused. Resume to fetch the latest entries." : snapshot ? `Last fetched ${observation(snapshot.observedAt)} · ${entries?.length} displayed` : "Connecting to Loki…"}</p>
    ...
    <p className="footnote">Loki · configured streams only. This rolling view refreshes after each request; busy streams may exceed the 100-entry window. Pause to read without updates.</p>
  </section>;
}
