...
export function podResources(pod: KubernetesPod, reading?: Reading<KubernetesPodMetrics[]>) {
  ...
  const timed = Boolean(sample?.window && !sample.error && ["Recent at sync", "Older at sync"].includes(freshness(sample.sampledAt, reading?.timestamp)));
  const complete = Boolean(timed && expected.length && expected.length === sample?.containers.length && expected.every(c => sample?.containers.filter(m => m.name === c.name).length === 1));
  ...
  return {
    ...
    state: sample ? metricState(sample, reading?.timestamp) : reading?.state === "unavailable" ? readingLabel(reading) : "Unavailable",
    containers: pod.containers.map(container => {
      const matches = sample?.containers.filter(c => c.name === container.name);
      const usage = timed && matches?.length === 1 ? matches[0] : null;
