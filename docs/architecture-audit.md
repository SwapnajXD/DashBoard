...
## Milestone live verification follow-up (2026-09-14)

The repository-only verification limits above describe the earlier audit. Subsequent authorized read-only checks confirmed Athena resource samples for eight cAdvisor containers and Hermes samples for five running pods / six containers. No infrastructure, exporter, Docker access, credentials or deployment configuration changed.

Athena exposes optional per-container `cpuCores` and `memoryWorkingSetBytes` readings through the existing Prometheus adapter. CPU uses the five-minute rate of the total CPU-seconds counter; memory uses working-set bytes. Queries retain the configured last-seen selector scope. Source/job/container identity is matched only on the server, and raw labels are discarded. Missing, ambiguous or stale source samples are unavailable independently for CPU and memory. Evaluation timestamps are labelled separately from last-seen telemetry; lifecycle and health-check state are unavailable.

Hermes preserves namespace, pod/container names, Metrics API timestamps, sampling windows and observation timestamps. Pod totals require complete matching container samples; individual container readings come directly from Metrics API entries. Samples older than two minutes become unavailable in the API and when the retained view ages. Duplicate inventory or metric identities are rejected. The two completed Helm-install pods returned no current metrics and remain unavailable. Existing filtering and table scrolling are preserved.

Final validation: the full test suite ran once. Four existing files passed; the newly retained cAdvisor check initially failed because its temporary-script top-level await was incompatible with the repository's CommonJS test transform. Wrapping it in an async `node:test` test fixed that harness error, and the targeted cAdvisor test then passed. All five test files are verified. Standalone TypeScript passed, and the single production build passed including TypeScript in a credential-free temporary copy. Whitespace checks passed; no standalone lint script/configuration is installed. The pre-existing `next-env.d.ts` content was preserved and excluded from the commits. No final browser or additional live infrastructure checks were run.
